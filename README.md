...
- **Parking area** — keep a skill in the library without giving it to any agent
...
- **Bilingual UI** — English by default, switch to 中文 in the top-right corner (remembered across sessions)
...
./start.sh --sandbox    # try it safely first: fake data, real files untouched (port 3210)

./dev.sh                # dev mode: no build step, hot reload (port 3000)
./dev.sh --sandbox      # dev mode + sandbox fake data (port 3210)

./stop.sh               # stop the running server
```

`PORT=4000 ./start.sh` picks a different port.

**Recommended first step: sandbox mode.** This tool moves / symlinks / copies / deletes files inside your agent directories, so play with a fake directory tree first (`--sandbox`), then point it at your real `$HOME` once you're comfortable.
...
| Action                       | Meaning                                                                                                                                    |
| ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| **Adopt**              | Move a scattered skill into the central library; the original location becomes a symlink (or copy)                                         |
| **Enable / Disable**   | Toggle a skill for one specific agent                                                                                                      |
| **Parking**            | In the library, but handed to no agent — saved for later                                                                                  |
| **Copy to my library** | Duplicate an agent's built-in skill as your own (copy only, original untouched)                                                            |
...
- What syncs: skill files + `manifest.json` (hashes, origins, tags, favorites). What doesn't: which agent uses which skill — each machine decides that for itself.
